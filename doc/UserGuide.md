...
To save the the current task queue to a *.silo-file, click <img src="pictures/document-save.svg" width="24" height="24"> or use the shortcut `[Ctrl]+[S]`.
...
The same goes for the *Add Exec* <img src="pictures/exec-add.svg" width="24" height="24"> button to insert a local execution tasks.
...
The *Connection* column shows with a online <img src="pictures/task-online.svg" width="24" height="24"> or a offline <img src="pictures/task-offline.svg" width="24" height="24"> symbol if the task is ready for execution. A neutral <img src="pictures/task-neutral.svg" width="24" height="24"> symbol (e.g. on delay-tasks) indicates that no network connection is necessary.
...
The columns *Start Time* as well as *End Time* contain timestamp entries for the begin and end of the task execution.

*Duration* shows the relative time a task took to finish.

The *Result* cells hold the outcome of each task execution. This can be a empty result `-`, a error message or a SiLA element nested in the cell indicated by `[...]`. Note: Empty braces `{}` are also considered valid result values.
...
According to the type of the current selected item, the panel in the bottom shows one the following
views.
